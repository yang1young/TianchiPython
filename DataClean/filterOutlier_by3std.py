...
# new_if1 = if1[np.abs(if1.qty_alipay_njhs-if1.qty_alipay_njhs.mean())<=(3*if1.qty_alipay_njhs.std())]

def filteredDataframe(if1,name):

    # for i,temp_df in if1.groupby([if1['cate_level_id']]):
    #     new_temp_fd = temp_df[np.abs(temp_df.qty_alipay_njhs-temp_df.qty_alipay_njhs.mean())>(3*temp_df.qty_alipay_njhs.std())]
    #     if1 = if1.drop(new_temp_fd.index)
    #     new_temp_fd.qty_alipay_njhs = if1.qty_alipay_njhs.mean()
    #     if1 = if1.append(new_temp_fd,ignore_index=True)
    #     print i,if1.qty_alipay_njhs.mean(),len(if1)
    #
    # print len(if1)
    #
    # for i,temp_df in if1.groupby([if1['cate_level_id'],if1['cate_id']]):
    #     new_temp_fd = temp_df[np.abs(temp_df.qty_alipay_njhs-temp_df.qty_alipay_njhs.mean())>(3*temp_df.qty_alipay_njhs.std())]
    #     if1 = if1.drop(new_temp_fd.index)
    #     new_temp_fd.qty_alipay_njhs = if1.qty_alipay_njhs.mean()
    #     if1 = if1.append(new_temp_fd,ignore_index=True)
